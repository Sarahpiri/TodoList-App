document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateTaskCounts() {
    ...
    if (pendingCount === 0) {
      pendingTextEl.textContent = "تسکی برای امروز ندارید!";
    } else {
      const persianPending = pendingCount.toLocaleString("fa-IR");
      pendingTextEl.textContent = `${persianPending} تسک را باید انجام دهید.`;
    }

    // Done task counter
    const doneCount = tasks.filter((t) => t.completed).length;
    const doneCountSpan = document.getElementById("done-count");
    doneCountSpan.textContent = doneCount.toLocaleString("fa-IR");
  }
  ...
  function createTaskElement(task) {
    ...
    taskElement.innerHTML = `
          ...
          <div class="task-actions relative self-start mt-2 sm:mt-0">
            ...
            <div class="hidden absolute top-full left-0 mt-[3px] w-[78px] h-[34px] p-[5px] border border-[#EBEDEF] dark:border-[--gray-800] rounded-[8px] shadow-[0px_12px_24px_-6px_#1414190F] flex items-center gap-2 z-10">
  ...
  function showEditForm(task) {
    ...
    editForm.innerHTML = `
      <input type="text" id="editTaskTitle" placeholder="نام تسک" class="w-full px-3 py-2 rounded-md sm:font-bold font-semibold sm:text-base text-sm text-[#7D7D7F] dark:text-white bg-[--background-light] focus:outline-none focus:ring-0" required value="${
        task.title
      }">
      <input type="text" id="editTaskDescription" placeholder="توضیحات" class="w-full px-3 py-2 rounded-md sm:font-semibold font-normal sm:text-sm text-xs text-[#AFAEB2] dark:text-[#83878F] bg-[--background-light] focus:outline-none focus:ring-0" value="${
        task.description
      }">
      ...
      <div id="editPriorityTags" class="hidden block w-56 right-0 top-full mt-4 mr-3 flex items-center gap-4 p-[10px] bg-[--surface-light] border border-[#EBEDEF] dark:border-[--gray-800] rounded-lg shadow-md z-50">
